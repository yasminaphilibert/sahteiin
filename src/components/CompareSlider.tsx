...
const CompareSlider = ({
  ...
  const left = withBase(leftSrc);
  const right = withBase(rightSrc);

  return (
    <div
      ref={rootRef}
      className={cn(
        "group relative w-full cursor-pointer overflow-hidden rounded-lg bg-white/5 select-none focus:outline-none focus-visible:ring-2 focus-visible:ring-amber/80",
        className
      )}
    ...
    >
      ...
      {/* Affordance: a dog-eared corner that lifts on hover. It is the only
          ...
          than repeated over every image. */}
      <div
        aria-hidden
        ...
        style={{
          ...
          background:
            "linear-gradient(225deg, rgba(244,240,247,0.92) 0%, rgba(226,220,232,0.9) 45%, rgba(150,142,160,0.9) 100%)",
